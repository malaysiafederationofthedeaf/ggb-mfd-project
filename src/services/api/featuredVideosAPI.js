...
import { FEATURED_VIDEOS_PLAYLIST_ID, YOUTUBE_API_KEY, YOUTUBE_BASE_URL } from "../../config";
...
export const fetchFeaturedVideosFromAPI = async () => {
  try {
    const response = await apiClient.get(
      `${window.location.origin}${YOUTUBE_PROXY_URL}`,
      { timeout: 15000 }
    );
    ...
    const isExpectedProxy404 =
      error.response?.status === 404 &&
      YOUTUBE_API_KEY &&
      FEATURED_VIDEOS_PLAYLIST_ID &&
      YOUTUBE_BASE_URL;

    if (!isExpectedProxy404) {
      console.error("Error fetching featured videos:", error);
    }

    // Local dev fallback when /api/youtube-playlist is not served locally
    if (isExpectedProxy404) {
      try {
        const fallbackResponse = await apiClient.get(
          `${YOUTUBE_BASE_URL}/playlistItems?part=snippet&maxResults=50&playlistId=${encodeURIComponent(
            FEATURED_VIDEOS_PLAYLIST_ID
        ...
        if (!fallbackResponse.data || !fallbackResponse.data.items) {
          console.error('Invalid fallback API response structure:', fallbackResponse);
          return [];
        }
        ...
        console.error("Fallback fetch featured videos failed:", fallbackError);
      }
    }
